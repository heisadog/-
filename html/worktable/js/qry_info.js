...
function createPage(result) {
    ...
    if(pageType=="Pay") {//生成客户欠款页面
        ...
        for (var i = 0; i < result.length; i++) {
            html += '<div class="">' +
                '<ul class="stock_head_sell">' +
                '<li style="width: 30%;text-indent: 12px;">' +
                '<span>' + result[i].xtxphm + '</span><br>' +
                '</li>' +
                '<li style="width: 25%;text-indent: 12px;">' + result[i].xsjssm + '</li>' +
                '<li style="width: 20%;">' + (result[i].kcxsje).toFixed(2) + '</li>' +
                '<li style="width: 25%;"><span style="color: #999">' + result[i].kcczrq + '</span></li>' +
                  '</ul>' +
                  '</div>';
        }
